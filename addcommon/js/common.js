...
function checkAnimation(user_scroll){
	...
	var offset = 150;
	if(ww >= 768){
		if(!$('body').hasClass('tablet') && !$('body').hasClass('shareholder_affairs') && !$('body').hasClass('policy-guideline')) {
...
// グループ企業採用
$(document).on('click', '.box.menu-group-company-recruitment', function(){
	$('.menu-group-company-recruitment .submenu').toggleClass("open");
});
$(document).on('click', 'header .bt_menu, header .gmenu .bg', function(){
	$('.menu-group-company-recruitment .submenu').removeClass('open');
});